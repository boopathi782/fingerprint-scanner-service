...
if(isset($_POST['submit']))
{
 ...
 if($count > 0)
 {
    $result1 = mysqli_query($conn, "SELECT fingerprint1,fingerprint2,fingerprint3,fingerprint4,fingerprint5 FROM register WHERE  id = 1 LIMIT 1");
    $row2 = mysqli_fetch_assoc($result1);
    /* echo "------------>".$row2['fingerprint'];
    echo "------------><br>".$row2['email_id']; */

    // echo "-------fingerprint1-----><br>".$row2['fingerprint1']; 
/*     echo "------fingerprint2------><br>".$row2['fingerprint2']; 
    echo "------fingerprint3------><br>".$row2['fingerprint3']; 
    echo "-------fingerprint4-----><br>".$row2['fingerprint4']; 
    echo "-------fingerprint5-----><br>".$row2['fingerprint5']; */ 
    
    if($row2['fingerprint1'] != ''){

    
            if($row2['fingerprint2'] == ''){
                $sql = "update register set fingerprint2 ='$txtIsoTemplate' where id = 1;";
                $res =  mysqli_query($conn, $sql) or die(mysqli_error($conn));
                    if($res){
                    echo "<script type='text/javascript'>alert('Succesfully fingerprint2 added..!!')</script>";
                    }
                    else
                    {
                        echo "<script type='text/javascript'>alert('fingerprint2 not able add..!!')</script>"; 
                    }
            }
            else if($row2['fingerprint3'] == ''){
                $sql = "update register set fingerprint3 ='$txtIsoTemplate' where id = 1;";
                $res =  mysqli_query($conn, $sql) or die(mysqli_error($conn));
                    if($res){
                    echo "<script type='text/javascript'>alert('Succesfully fingerprint3 added..!!')</script>";
                    }
                    else
                    {
                        echo "<script type='text/javascript'>alert('fingerprint3 not able add..!!')</script>"; 
                    }
            }
            else if($row2['fingerprint4'] == ''){
                $sql = "update register set fingerprint4 ='$txtIsoTemplate' where id = 1;";
                $res =  mysqli_query($conn, $sql) or die(mysqli_error($conn));
                    if($res){
                    echo "<script type='text/javascript'>alert('Succesfully fingerprint4 added..!!')</script>";
                    }
                    else
                    {
                        echo "<script type='text/javascript'>alert('fingerprint4 not able add..!!')</script>"; 
                    }
            }
            else if($row2['fingerprint5'] == ''){
                $sql = "update register set fingerprint5 ='$txtIsoTemplate' where id = 1;";
                $res =  mysqli_query($conn, $sql) or die(mysqli_error($conn));
                    if($res){
                    echo "<script type='text/javascript'>alert('Succesfully fingerprint5 added..!!')</script>";
                    }
                    else
                    {
                        echo "<script type='text/javascript'>alert('fingerprint5 not able add..!!')</script>"; 
                    }
            } 
...
$sql = "insert into register(email_id, password , confirm_password ,activation , ip , date , fingerprint1)
        values('$exampleInputEmail1', '1234' ,'1234', '1234' , '1234' , '$my_date' , '$txtIsoTemplate');";
...
//function to capture the finger prints. 
        ...
        {
          alert("password should be max 3 char");
          return false;
        }
